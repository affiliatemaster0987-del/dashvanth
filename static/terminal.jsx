...
const KRT_BUILD = "v31-1788842602";
...
function useFeed() {
  ...
  return {
    ...
    candleHealth: (snap && snap.candle_health) || null,
...
/* ---------------- break log ----------------
   A timeline, not a snapshot. Each row is the first time a level was taken
   today, newest at the top, with the contract that expresses it. */
function BreakRow({ e }) {
  const [open, setOpen] = useState(false);
  const up = e.side === "CE";
  const col = up ? C.bull : C.bear;
  const moved = has(e.move_pct);
  return (
    <div style={{ borderBottom: `1px solid ${C.lineSoft}`, padding: "10px 12px",
      borderLeft: `3px solid ${col}` }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 8, flexWrap: "wrap" }}>
        <div style={{ minWidth: 0 }}>
          <span className="mono" style={{ fontSize: 13, color: C.saffron }}>{text(e.at)}</span>
          <span style={{ fontSize: 14, fontWeight: 700, marginLeft: 8 }}>{text(e.sym)}</span>
          <span style={{ color: col, marginLeft: 6, fontWeight: 700 }}>{text(e.side, "")}</span>
          <div className="mono" style={{ fontSize: 10.5, color: C.dim, marginTop: 3 }}>
            {text(e.level_name)} {rupee(e.level)} broken
            {has(e.vol_ratio) ? ` · ${num(e.vol_ratio, 1)}× volume` : " · volume not reported"}
            {e.vwap_ok === true ? " · above VWAP" : e.vwap_ok === false ? " · below VWAP" : ""}
            {e.sector ? ` · ${text(e.sector)}` : ""}
          </div>
        </div>
        <div style={{ textAlign: "right" }}>
          <div className="mono" style={{ fontSize: 12 }}>{rupee(e.ltp)}</div>
          {moved && (
            <div className="mono" style={{ fontSize: 10.5,
              color: e.move_pct > 0 ? C.bull : e.move_pct < 0 ? C.bear : C.dim }}>
              {e.move_pct > 0 ? "+" : ""}{num(e.move_pct, 2)}% since break
            </div>
          )}
        </div>
      </div>

      {e.option && has(e.option.prem) ? (
        <>
          <div className="mono" style={{ fontSize: 11, color: C.text, marginTop: 6 }}>
            {text(e.option.symbol)} · {rupee(e.option.prem)}
          </div>
          <button onClick={() => setOpen(!open)} className="mono" style={{
            marginTop: 6, fontSize: 9.5, letterSpacing: ".1em", color: C.saffron,
            padding: "3px 9px", border: `1px solid ${C.saffron}55`, borderRadius: 2 }}>
            {open ? "HIDE PLAN" : "SHOW PLAN"}
          </button>
          {open && <OptionPlan opt={e.option} score={70} />}
        </>
      ) : (
        <div className="mono" style={{ fontSize: 10, color: C.faint, marginTop: 6 }}>
          No contract quoted for this break.
        </div>
      )}
    </div>
  );
...
function BreaksTab({ feed }) {
  const snap = feed.snap;
  const [side, setSide] = useState("ALL");
  if (!snap) return <WarmingPanel feed={feed} />;

  const br = snap.breaks;
  if (!br || !br.count) {
    return (
      <Panel title="BREAK LOG">
        <Empty>No level has been taken yet today. Rows appear here the moment price crosses a
          previous day, week or month extreme, or either opening range.</Empty>
      </Panel>
    );
  }

  const ce = br.ce || [], pe = br.pe || [];
  const List = ({ rows, label, col }) => (
    <Panel title={`${label} · ${rows.length}`} pad={0}
      right={<span className="mono" style={{ fontSize: 9.5, color: col }}>NEWEST FIRST</span>}>
      {rows.length ? rows.map((e, i) => <BreakRow key={`${e.sym}-${e.key}-${i}`} e={e} />)
        : <Empty>Nothing on this side yet.</Empty>}
    </Panel>
  );

  return (
    <div style={{ display: "grid", gap: 12 }}>
      <Panel title="BREAK LOG" pad={12}
        right={<span className="mono" style={{ fontSize: 9.5, color: C.dim }}>{br.count} TODAY</span>}>
        <div style={{ display: "flex", gap: 6, marginBottom: 10 }}>
          {["ALL", "CE", "PE"].map((k) => (
            <button key={k} onClick={() => setSide(k)} className="mono" style={{
              fontSize: 10, padding: "4px 12px", borderRadius: 2,
              border: `1px solid ${side === k ? C.saffron : C.line}`,
              color: side === k ? C.saffron : C.dim }}>{k}</button>
          ))}
        </div>
        <div className="mono" style={{ fontSize: 10.5, color: C.dim, lineHeight: 1.65 }}>
          {text(br.note)} A break is not by itself a trade — check the volume beside it and the
          plan under it before acting.
        </div>
      </Panel>

      {(side === "ALL" || side === "CE") && <List rows={ce} label="CE LIST · BREAKOUTS" col={C.bull} />}
      {(side === "ALL" || side === "PE") && <List rows={pe} label="PE LIST · BREAKDOWNS" col={C.bear} />}
    </div>
  );
}
...
/* ---------------- gamma move ---------------- */
const GSTATE = {
  ACTIVE: ["🚀 MOMENTUM ACTIVE", C.bull],
  WEAKENING: ["⚠️ MOMENTUM WEAKENING", C.saffron],
  FAILED: ["🔴 GAMMA MOVE FAILED · EXIT", C.bear],
  COMPLETED: ["✅ ALL TARGETS DONE", C.bull],
};
const gBand = (b) =>
  b === "STRONG GAMMA MOVE" ? C.bull : b === "GAMMA TRIGGER" ? C.saffron :
  b === "GAMMA WATCH" ? C.silver : b === "INSUFFICIENT DATA" ? C.bear : C.dim;

function GammaCard({ g }) {
  const [open, setOpen] = useState(false);
  const o = g.option || {};
  const L = g.live;
  const c = g.case || {};
  const st = L && GSTATE[L.state];

  return (
    <div style={{ border: `1px solid ${gBand(g.band)}55`, borderRadius: 4, padding: 13,
      marginBottom: 12, background: `${gBand(g.band)}0A` }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 8, flexWrap: "wrap" }}>
        <div>
          <span style={{ fontSize: 17, fontWeight: 700 }}>{text(g.sym)}</span>
          <span style={{ color: g.side === "CE" ? C.bull : C.bear, marginLeft: 8, fontWeight: 700 }}>
            {g.side === "CE" ? "BULLISH" : "BEARISH"}
          </span>
          <div className="mono" style={{ fontSize: 10.5, color: C.dim, marginTop: 3 }}>
            {L ? `signal ${text(L.at)} · ` : ""}spot {rupee(g.spot)}
            {g.sector ? ` · ${text(g.sector)}` : ""}
          </div>
        </div>
        <div style={{ textAlign: "right" }}>
          <div className="mono" style={{ fontSize: 22, color: gBand(g.band) }}>
            {has(g.score) ? g.score : DASH}<span style={{ fontSize: 11, color: C.dim }}>/100</span>
          </div>
          <Tag color={gBand(g.band)}>{text(g.band)}</Tag>
        </div>
      </div>

      <div className="mono" style={{ fontSize: 10, color: C.dim, letterSpacing: ".12em", marginTop: 12 }}>
        CONFIRMATION
      </div>
      {/* The direction word comes from the side being traded. Reading it off a
          boolean printed "Above VWAP" on a bearish call, which is the opposite
          of what the engine had actually checked. */}
      <div className="mono" style={{ fontSize: 10.5, color: C.bull, lineHeight: 1.7 }}>
        <span style={{ width: 16, display: "inline-block" }}>✓</span>
        {text(c.vwap_side, g.side === "CE" ? "above" : "below").replace(/^./, (m) => m.toUpperCase())}
        {" VWAP "}{rupee(c.vwap)}
      </div>
      {(c.broken || []).map((b, i) => (
        <div key={i} className="mono" style={{ fontSize: 10.5, color: C.bull, lineHeight: 1.7 }}>
          <span style={{ width: 16, display: "inline-block" }}>✓</span>
          {text(b.name)} {g.side === "CE" ? "breakout" : "breakdown"} {rupee(b.level)}
        </div>
      ))}
      <div className="mono" style={{ fontSize: 10.5, lineHeight: 1.7,
        color: c.trend_ok ? C.bull : C.dim }}>
        <span style={{ width: 16, display: "inline-block" }}>{c.trend_ok ? "✓" : "·"}</span>
        {c.trend_ok ? "Momentum agrees" : "Momentum not confirmed"}
        {c.stack ? ` · ${text(c.stack)}` : ""}
      </div>
      <div className="mono" style={{ fontSize: 10.5, lineHeight: 1.7,
        color: ["GOOD", "STRONG"].includes(c.vol_band) ? C.bull : C.saffron }}>
        <span style={{ width: 16, display: "inline-block" }}>
          {["GOOD", "STRONG"].includes(c.vol_band) ? "✓" : "·"}
        </span>
        Relative volume {text(c.vol_note, DASH)}
        {c.vol_band ? ` (${text(c.vol_band)})` : ""}
      </div>
      ...
      <div style={{ fontSize: 14, fontWeight: 700, marginTop: 3 }}>
        {text(g.sym)} {num(o.strike, 0)} {text(g.side, "")}
      </div>
      <Row label="OPTION LTP" value={rupee(L ? L.prem : o.prem)} />
      <Row label="DELTA" value={has(o.delta) ? num(o.delta, 3) : DASH} />
      <Row label="GAMMA" value={has(o.gamma) ? o.gamma.toFixed(6) : DASH} />
      <Row label="OPTION VOLUME" value={has(o.vol) ? intl(o.vol) : DASH} />
      <Row label="SPREAD" value={has(o.spread_pct) ? `${num(o.spread_pct, 2)}% of premium` : DASH}
        color={o.spread_pct <= 1.5 ? C.bull : C.saffron} />
      <Row label="EXPIRY" value={text(o.expiry)} />

      {L && L.plan && (
        <>
          <div className="mono" style={{ fontSize: 10, color: C.dim, letterSpacing: ".12em", marginTop: 12 }}>
            PLAN · FROM THE SIGNAL PRICE {rupee(L.signal_prem)}
          </div>
          <div className="mono" style={{ fontSize: 11.5, lineHeight: 1.9, marginTop: 4 }}>
            {[["T1", L.plan.t1], ["T2", L.plan.t2], ["T3", L.plan.t3]].map(([k, v]) => {
              const at = L.hits && L.hits[k.toLowerCase()];
              return (
                <div key={k} style={{ color: at ? C.bull : C.dim }}>
                  <span style={{ width: 20, display: "inline-block" }}>{at ? "✅" : "⏳"}</span>
                  {k} {rupee(v)}{at ? ` — HIT ${text(at)}` : " — running"}
                </div>
              );
            })}
            <div style={{ color: L.hits && L.hits.sl ? C.bear : C.dim }}>
              <span style={{ width: 20, display: "inline-block" }}>{L.hits && L.hits.sl ? "❌" : "·"}</span>
              SL {rupee(L.plan.sl)}{L.hits && L.hits.sl ? ` — HIT ${text(L.hits.sl)}` : ""}
            </div>
          </div>
          <div className="mono" style={{ fontSize: 10.5, color: C.dim, marginTop: 8 }}>
            entry {rupee(L.signal_prem)} → now {rupee(L.prem)}
            {has(L.pnl_pct) && <span style={{ color: L.pnl_pct >= 0 ? C.bull : C.bear }}>
              {" "}({L.pnl_pct >= 0 ? "+" : ""}{num(L.pnl_pct, 1)}%)</span>}
            {" · peak "}{rupee(L.peak)}
          </div>
        </>
      )}
      {L && !L.plan && L.plan_why && (
        <div className="mono" style={{ fontSize: 10, color: C.saffron, marginTop: 8, lineHeight: 1.55 }}>
          {text(L.plan_why)}
        </div>
      )}

      {st && (
        <div className="mono" style={{ fontSize: 11.5, color: st[1], marginTop: 12, letterSpacing: ".08em" }}>
          {st[0]}
          {L.failed_at ? ` — ${text(L.failed_at)}` : L.weak_at ? ` — ${text(L.weak_at)}` : ""}
        </div>
      )}
      {L && L.fail_reason && (
        <div className="mono" style={{ fontSize: 10, color: C.dim, marginTop: 4, lineHeight: 1.55 }}>
          {text(L.fail_reason)}
        </div>
      )}

      {g.cap_note && (
        <div className="mono" style={{ fontSize: 10, color: C.saffron, marginTop: 10, lineHeight: 1.55 }}>
          Score capped at {g.capped_at} from a raw {g.raw_score}. {text(g.cap_note)}
        </div>
      )}
      ...
      <button onClick={() => setOpen(!open)} className="mono" style={{
        marginTop: 10, fontSize: 9.5, letterSpacing: ".1em", color: C.saffron,
        padding: "4px 10px", border: `1px solid ${C.saffron}55`, borderRadius: 2 }}>
        {open ? "HIDE SCORE" : "SCORE BREAKDOWN"}
      </button>
      {open && (
        <div style={{ marginTop: 10 }}>
          {(g.components || []).map((k) => (
            <div key={k.k} style={{ display: "grid", gridTemplateColumns: "104px 1fr 44px", gap: 8,
              alignItems: "center", padding: "3px 0" }}>
              <span className="mono" style={{ fontSize: 9.5, color: k.available ? C.dim : C.faint }}>
                {text(k.k)}
              </span>
              {k.available
                ? <Meter v={k.earned} max={k.weight} color={k.earned >= k.weight ? C.bull : k.earned > 0 ? C.saffron : C.bear} />
                : <div style={{ height: 3, background: `repeating-linear-gradient(90deg, ${C.lineSoft} 0 4px, transparent 4px 8px)` }} />}
              <span className="mono" style={{ fontSize: 9.5, textAlign: "right", color: k.available ? C.dim : C.faint }}>
                {k.available ? `${k.earned}/${k.weight}` : "n/a"}
              </span>
              <span className="mono" style={{ gridColumn: "1 / -1", fontSize: 9, color: C.faint }}>
                {text(k.detail, "")}
              </span>
            </div>
          ))}
          {(o.compared || []).length > 1 && (
            <>
              <div className="mono" style={{ fontSize: 10, color: C.dim, letterSpacing: ".12em", marginTop: 10 }}>
                STRIKES COMPARED
              </div>
              {o.compared.map((k, i) => (
                <div key={i} className="mono" style={{ fontSize: 10, lineHeight: 1.7,
                  color: k.strike === o.strike ? C.bull : C.dim }}>
                  {k.strike === o.strike ? "→ " : "   "}{num(k.strike, 0)} · {rupee(k.prem)}
                  {has(k.delta) ? ` · delta ${num(k.delta, 2)}` : ""}
                  {has(k.spread_pct) ? ` · spread ${num(k.spread_pct, 2)}%` : ""}
                  {` · fit ${num(k.fit, 2)}`}
                </div>
              ))}
            </>
...
function GammaTab({ feed }) {
  const snap = feed.snap;
  if (!snap) return <WarmingPanel feed={feed} />;
  const g = snap.gamma;
  if (!g || g.available === false) {
    return <Panel title="⚡ KRT GAMMA MOVE">
      <Empty>{text(g && g.note, "The gamma scanner has not run on this snapshot.")}</Empty>
    </Panel>;
  }

  const qual = g.qualified || [];
  const all = g.signals || [];
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      {g.best ? (
        <Panel title="🏆 GAMMA CALL OF THE DAY" pad={12}
          right={<span className="mono" style={{ fontSize: 9.5, color: C.dim }}>ALL CHECKS PASSED</span>}>
          <GammaCard g={g.best} />
          {(g.best_checks || []).length > 0 && (
            <div style={{ marginTop: 4 }}>
              <div className="mono" style={{ fontSize: 9.5, color: C.dim, letterSpacing: ".12em", marginBottom: 4 }}>
                WHY IT EARNED THE SLOT — SCORE ALONE DOES NOT
              </div>
              {(g.best_checks || []).map((k, i) => (
                <div key={i} className="mono" style={{ fontSize: 10.5, color: C.bull, lineHeight: 1.7 }}>
                  <span style={{ width: 16, display: "inline-block" }}>✓</span>
                  {text(k.k)}<span style={{ color: C.faint }}> · {text(k.note)}</span>
                </div>
              ))}
            </div>
          )}
        ...
        <Panel title="⚡ KRT GAMMA MOVE" pad={12}>
          <div className="mono" style={{ fontSize: 12, color: C.saffron, letterSpacing: ".08em" }}>
            NO QUALIFIED GAMMA MOVE
          </div>
          <div className="mono" style={{ fontSize: 10.5, color: C.dim, marginTop: 6, lineHeight: 1.6 }}>
            {text(g.note)}
          </div>
          {(g.day_call_rejected || []).length > 0 && (
            <div style={{ marginTop: 10 }}>
              <div className="mono" style={{ fontSize: 9.5, color: C.dim, letterSpacing: ".12em", marginBottom: 4 }}>
                SCORED, BUT NOT PROMOTED
              </div>
              {(g.day_call_rejected || []).map((r, i) => (
                <div key={i} className="mono" style={{ fontSize: 10.5, color: C.dim, lineHeight: 1.7 }}>
                  {text(r.sym)} <span style={{ color: r.side === "CE" ? C.bull : C.bear }}>{text(r.side, "")}</span>
                  {" "}{r.score} — failed: <span style={{ color: C.bear }}>{(r.failed || []).join(", ")}</span>
                </div>
              ))}
            </div>
          )}
      ...
      {qual.length > 1 && (
        <Panel title="🔥 LIVE GAMMA MOVES" pad={0}>
          {qual.map((x, i) => (
            <div key={i} style={{ display: "grid", gridTemplateColumns: "26px 1fr 50px 96px",
              gap: 8, alignItems: "center", padding: "9px 12px", borderBottom: `1px solid ${C.lineSoft}` }}>
              <span className="mono" style={{ fontSize: 10, color: C.saffron }}>#{i + 1}</span>
              <span style={{ fontSize: 12.5 }}>
                {text(x.sym)} {num((x.option || {}).strike, 0)}{" "}
                <span style={{ color: x.side === "CE" ? C.bull : C.bear }}>{text(x.side, "")}</span>
              </span>
              <span className="mono" style={{ fontSize: 13, textAlign: "right", color: gBand(x.band) }}>
                {x.score}
              </span>
              <span className="mono" style={{ fontSize: 9, textAlign: "right", color: gBand(x.band) }}>
                {text(x.band)}
              </span>
            </div>
          ))}
      ...
      {all.filter((x) => !qual.includes(x)).length > 0 && (
        <Panel title="FORMING · NOT YET A TRIGGER" pad={0}>
          {all.filter((x) => !qual.includes(x)).map((x, i) => (
            <div key={i} style={{ padding: "8px 12px", borderBottom: `1px solid ${C.lineSoft}` }}>
              <span style={{ fontSize: 12 }}>
                {text(x.sym)} <span style={{ color: x.side === "CE" ? C.bull : C.bear }}>{text(x.side, "")}</span>
              </span>
              <span className="mono" style={{ fontSize: 11, color: gBand(x.band), float: "right" }}>
                {has(x.score) ? x.score : DASH} · {text(x.band)}
              </span>
              <div className="mono" style={{ fontSize: 9.5, color: C.faint, marginTop: 2 }}>
                {text(x.stage_label)}
                {x.coverage_note ? ` · ${text(x.coverage_note)}` : ""}
              </div>
            </div>
          ))}
        </Panel>
      ...
      {carried.length > 0 && (
        <Panel title="EARLIER SIGNALS TODAY" pad={0}
          right={<span className="mono" style={{ fontSize: 9.5, color: C.dim }}>KEPT, INCLUDING FAILURES</span>}>
          {carried.map((L, i) => {
            const st = GSTATE[L.state] || GSTATE.FAILED;
            return (
              <div key={i} style={{ padding: "9px 12px", borderBottom: `1px solid ${C.lineSoft}` }}>
                <div className="mono" style={{ fontSize: 11 }}>
                  <span style={{ color: C.saffron }}>{text(L.at)}</span>{" "}
                  {text(L.sym)} <span style={{ color: L.side === "CE" ? C.bull : C.bear }}>{text(L.side, "")}</span>
                  {" "}{num(L.strike, 0)} · entry {rupee(L.signal_prem)} → {rupee(L.prem)}
                  {has(L.pnl_pct) && <span style={{ color: L.pnl_pct >= 0 ? C.bull : C.bear }}>
                    {" "}({L.pnl_pct >= 0 ? "+" : ""}{num(L.pnl_pct, 1)}%)</span>}
                </div>
                <div className="mono" style={{ fontSize: 9.5, color: st[1], marginTop: 2 }}>
                  {st[0]}
                  {L.hits && Object.keys(L.hits).length
                    ? " · " + Object.entries(L.hits).map(([k, t]) => `${k.toUpperCase()} ${t}`).join(" · ")
                    : ""}
                </div>
              </div>
            );
      ...
      <Panel title="HOW THIS IS DECIDED">
        <div className="mono" style={{ fontSize: 10.5, color: C.dim, lineHeight: 1.7 }}>
          {text(g.method)}
        </div>
        <div className="mono" style={{ fontSize: 10, color: C.dim, marginTop: 10, lineHeight: 1.6 }}>
          Delta and gamma are solved from the quoted premium with Black-Scholes — the broker sends
          no greeks, so they are arithmetic on a price rather than a second source. "Momentum" here
          is price and volume behaviour across scans, not order flow: the aggressor side of each
          trade is not in a REST quote and is never guessed at.
        </div>
      </Panel>
    </div>
  );
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      <Panel title="FEED HEALTH">
        ...
        <Row label="CANDLES OK / FAILED"
          value={feed.candleHealth ? `${intl(feed.candleHealth.ok)} / ${intl(feed.candleHealth.failed)}` : DASH}
          color={feed.candleHealth && feed.candleHealth.failed ? C.bear : C.bull} />
        <Row label="LAST CANDLE REFUSAL"
          value={feed.candleHealth && feed.candleHealth.last_error
            ? text(feed.candleHealth.last_error.message) : "none"}
          color={feed.candleHealth && feed.candleHealth.last_error ? C.bear : C.dim} />
...
const TABS = [
  ...
  { id: "breaks", label: "BREAKS", sub: "CE / PE with time" },
  { id: "gamma", label: "GAMMA", sub: "stock option moves" },
      ...
      <div className="krt-shell">
        ...
        <main style={{ minWidth: 0 }}>
          ...
          <div style={{ padding: 12 }}>
            ...
            {tab === "breaks" && <BreaksTab feed={feed} />}
            {tab === "gamma" && <GammaTab feed={feed} />}
