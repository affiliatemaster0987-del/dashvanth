...
def full_scan() -> dict:
    ...
    # "Check SYSTEM" is a worse answer than the numbers themselves. Each stage
    # is counted here so the banner can name the one that actually broke.
    n_universe = len(config.UNIVERSE)
    n_tokens = sum(1 for sym in config.UNIVERSE if client.token_for(sym))
    ready = bool(getattr(client, "instruments_ready", lambda: True)())
    ...
    if not stocks and not indices:
        ...
        if not ready or n_tokens == 0:
            empty_reason = (
                ...
                "instrument master is either still downloading or failed to load. It usually "
                f"clears a minute or two after a restart. [{stage}]")
    ...
    elif indices and not stocks:
        empty_reason = (f"Indices scanned but none of the {n_universe} stocks did "
