...
const KRT_BUILD = "v23-1788239012";
...
function FlowTab({ feed }) {
      ...
      <div style={{ padding: "12px 14px", borderBottom: `1px solid ${C.lineSoft}` }}>
        ...
        {o && has(o.prem) ? (
          <div style={{ marginTop: 8 }}>
            <div className="mono" style={{ fontSize: 10, color: C.dim, letterSpacing: ".1em" }}>BEST OPTION</div>
            <div className="mono" style={{ fontSize: 12.5, color: C.text, marginTop: 2 }}>
              {num(o.strike, 0)} {text(c.side, "")} · {rupee(o.prem)}
            </div>
            <OptionPlan opt={o} score={c.score} compact />
          </div>
        ) : (
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      {best && best.option && has(best.option.prem) && (
...
function AccumTab({ feed }) {
  ...
  const Map = ({ side }) => {
    ...
    const topClean = rows.find((r) => !(r.traps || []).length && !r.thin);
    return (
      <div style={{ marginBottom: 12 }}>
        ...
        {rows.map((r, i) => {
          ...
          const isBest = topClean && r.strike === topClean.strike;
          return (
            <button key={i} onClick={() => setOpen(open === `${side}${r.strike}` ? null : `${side}${r.strike}`)}
              style={{ display: "grid", gridTemplateColumns: "1fr 46px 84px", gap: 8, width: "100%",
              ...
              <span className="mono" style={{ fontSize: 9, textAlign: "right", color: trap ? C.bear : b.color }}>
                {trap ? "⚠ TRAP" : isBest ? "🔥 BEST" : `${b.icon} ${b.label}`}
              </span>
              {open === `${side}${r.strike}` && (
