...
function toSetup(r, i) {
  ...
  const passed = checks.filter((c) => c.ok && c.note).map((c) => c.note);
  const blocked = checks.filter((c) => !c.ok && c.note).map((c) => c.note);
  const level = isCE ? lv.pdh : lv.pdl;

  return {
    id: `${r.sym}-${r.side}-${i}`,
    sym: r.sym, side: r.side, sector: r.sector,
    ...
    stack: r.stack, confidence: r.confidence, grade: r.grade,
...
const Row = ({ label, value, color }) => (
  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 10, padding: "5px 0", borderBottom: `1px solid ${C.lineSoft}` }}>
    ...
    <span className="mono" style={{ fontSize: 12.5, color: value === DASH ? C.faint : (color || C.text), fontWeight: 500, textAlign: "right" }}>{value}</span>
  </div>
);
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
        <div>
          <div style={{ fontSize: 15, fontWeight: 700, letterSpacing: "-.01em" }}>
            {s.sym} <span style={{ color: s.side === "CE" ? C.bull : C.bear }}>{s.side}</span>
          </div>
...
function SetupDetail({ s }) {
  ...
  return (
    <div className="krt-in">
      <Panel title={`SETUP DOSSIER · ${s.sym} ${s.side}`} right={<Tag color={tierColor(s.tier)} solid>{s.tier}</Tag>}>
        <div style={{ display: "flex", alignItems: "baseline", gap: 10, marginBottom: 12, flexWrap: "wrap" }}>
          ...
          <div>
            ...
            <div className="mono" style={{ fontSize: 10.5, color: C.saffron }}>
              Stage · {STAGES[s.stage]}{s.grade ? ` · Grade ${s.grade}` : ""}{s.confidence ? ` · ${s.confidence}` : ""}
            </div>
          </div>
        </div>
      ...
      <Panel title="LEVELS ON THE TAPE" style={{ marginTop: 12 }}>
        ...
        <Row label="EMA STACK" value={s.stack || DASH} />
      ...
      <Panel title="BREAK CONFIRMATION" style={{ marginTop: 12 }}>
        {s.fake ? (
          <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
            ...
            <span className="mono" style={{ fontSize: 11, color: C.dim }}>{s.fake.msg}</span>
          </div>
      ...
      <Panel title="MARKET READ" right={<span className="mono krt-live" style={{ fontSize: 9.5, color: C.saffron }}>● {feed.stale ? "STALE" : "LIVE"}</span>}>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 12, flexWrap: "wrap" }}>
          <Tag color={decision.side === "CE" ? C.bull : decision.side === "PE" ? C.bear : C.saffron}>
            SIDE · {decision.side || DASH}
          </Tag>
          {has(decision.edge) && <Tag color={C.dim}>EDGE · {num(decision.edge, 0)}</Tag>}
          {decision.best_sector && <Tag color={C.gold}>LEAD · {decision.best_sector}</Tag>}
          {decision.sector_pick && (
            <Tag color={C.saffron}>PICK · {decision.sector_pick.sym} {decision.sector_pick.side}</Tag>
          )}
        </div>
        {decision.why && (
          <div className="mono" style={{ fontSize: 10.5, color: C.dim, marginTop: 10, lineHeight: 1.6 }}>{decision.why}</div>
        )}
      </Panel>
              ...
              <>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 10, gap: 8 }}>
                  <div style={{ fontSize: 15, fontWeight: 700 }}>
                    {armed.sym} <span className="mono" style={{ fontSize: 12, color: C.dim, fontWeight: 400 }}>{rupee(armed.ltp)}</span>
                  </div>
                  <Tag color={C.saffron} solid>{armed.side}</Tag>
                </div>
                <Row label="SCORE" value={armed.score} color={C.saffron} />
                <Row label="TIER" value={armed.tier || DASH} />
                <Row label="SECTOR" value={armed.sector || DASH} />
                <Row label={armed.level_name || "PENDING LEVEL"} value={rupee(armed.level)} color={C.saffron} />
                ...
                <div className="mono" style={{ fontSize: 10.5, color: C.dim, marginTop: 10, lineHeight: 1.5 }}>
                  {armed.blocking ? `Blocked by: ${armed.blocking}` : "Price has not taken the level yet. Touching is not breaking."}
                </div>
              </>
              ...
              <div style={{ maxHeight: 340, overflowY: "auto" }}>
                {leaders.map((r, i) => (
                  <div key={r.sym + i} style={{
                    ...
                    <span style={{ fontSize: 12.5, fontWeight: 500 }}>{r.sym}</span>
                  ...
                  return (
                    <div key={r.name} style={{ marginBottom: 9 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3, gap: 8 }}>
                        <span className="mono" style={{ fontSize: 10.5, color: C.text }}>{r.name}</span>
                ...
                <div key={n.sym + n.side + i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, padding: "8px 12px", borderBottom: `1px solid ${C.lineSoft}` }}>
                  <div style={{ minWidth: 0 }}>
                    <div className="mono" style={{ fontSize: 11, color: C.text }}>
                      {n.sym} <span style={{ color: n.side === "CE" ? C.bull : C.bear }}>{n.side}</span>
                      <span style={{ color: C.dim }}> · {rupee(n.ltp)}</span>
                    </div>
                    <div className="mono" style={{ fontSize: 9.5, color: C.bear, marginTop: 2 }}>{n.blocking || DASH}</div>
                  </div>
...
function FlowTab({ feed }) {
  ...
  const loadChain = useCallback(async (sym, side) => {
    setLoading(true); setChainErr(null); setChain(null); setChainSym(sym);
    try {
      const res = await fetch(`/api/chain/${sym}?side=${side || "CE"}`, { cache: "no-store" });
      const body = await res.json();
    ...
    <div style={{ display: "grid", gap: 12 }}>
      <Panel title="INDEX OPTION CARDS" pad={0}
        ...
        {cards.length ? cards.map((c) => {
          const o = c.option;
          return (
            <div key={c.sym} style={{ padding: "12px 14px", borderBottom: `1px solid ${C.lineSoft}` }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <span style={{ fontSize: 15, fontWeight: 700 }}>{c.sym}</span>
              ...
              {c.pcr_read && (
                <div className="mono" style={{ fontSize: 10.5, color: C.saffron, marginTop: 8 }}>{c.pcr_read}</div>
              )}
              {c.note && (
                <div className="mono" style={{ fontSize: 10.5, color: C.dim, marginTop: 6 }}>{c.note}</div>
              )}
      ...
      {chainSym && (
        <Panel title={`LIVE CHAIN · ${chainSym}`} pad={0}
      ...
      <Panel title="INDEX RADAR" pad={0}>
        {radar.length ? radar.map((r, i) => (
          <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr 60px 60px 80px", gap: 8, alignItems: "center", padding: "8px 14px", borderBottom: `1px solid ${C.lineSoft}` }}>
            <span style={{ fontSize: 12 }}>{r.sym}</span>
            <span className="mono" style={{ fontSize: 11, color: r.side === "CE" ? C.bull : C.bear, textAlign: "right" }}>{r.side}</span>
            ...
            <span className="mono" style={{ fontSize: 10, textAlign: "right", color: r.qualified ? C.bull : C.faint }}>
              {r.qualified ? "QUALIFIED" : `GRADE ${r.grade || DASH}`}
            </span>
          </div>
      ...
      {rows && rows.length ? rows.map((r, i) => (
        <div key={r.sym + i} style={{ padding: "9px 12px", borderBottom: `1px solid ${C.lineSoft}` }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
            <span style={{ fontSize: 13, fontWeight: 600 }}>{r.sym}</span>
          ...
          <div className="mono" style={{ fontSize: 9.5, color: C.dim, marginTop: 5, lineHeight: 1.5 }}>
            {r.conf}{r.sector ? ` · ${r.sector}` : ""}{has(r.vol_ratio) ? ` · ${num(r.vol_ratio, 1)}× vol` : ""}
          </div>
          {r.hits && r.hits.length > 0 && (
            <div className="mono" style={{ fontSize: 9, color: C.faint, marginTop: 3 }}>{r.hits.join(" · ")}</div>
          )}
        </div>
          ...
          <div key={s.sym + i} style={{
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ fontSize: 14, fontWeight: 700 }}>{s.sym}</span>
            ...
            {breakout.map((r, i) => (
              <div key={r.sym + i} style={{
                ...
                <span style={{ fontSize: 12 }}>
                  {r.sym} {r.armed && <span className="mono" style={{ fontSize: 8.5, color: C.saffron }}>ARMED</span>}
                </span>
                <span className="mono" style={{ fontSize: 9.5, color: C.dim }}>{r.name || DASH}</span>
        ...
        <div className="krt-cols3" style={{ gap: 10 }}>
          <div style={{ background: C.raised, border: `1px solid ${C.lineSoft}`, borderRadius: 3, padding: "10px 12px" }}>
            ...
            <div className="mono" style={{ fontSize: 11.5, color: C.bull, marginTop: 4, lineHeight: 1.5 }}>
              {(bias.positive || []).length ? bias.positive.join(" · ") : DASH}
            </div>
          </div>
          <div style={{ background: C.raised, border: `1px solid ${C.lineSoft}`, borderRadius: 3, padding: "10px 12px" }}>
            ...
            <div className="mono" style={{ fontSize: 11.5, color: C.bear, marginTop: 4, lineHeight: 1.5 }}>
              {(bias.negative || []).length ? bias.negative.join(" · ") : DASH}
            </div>
          </div>
          <div style={{ background: C.raised, border: `1px solid ${C.lineSoft}`, borderRadius: 3, padding: "10px 12px" }}>
          ...
          <div key={i} style={{
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                ...
                {(n.symbols || []).map((s) => (
                  <span key={s} style={{ fontSize: 13, fontWeight: 600 }}>{s}</span>
                ))}
                <Tag color={toneCol(n.bias)}>{n.bias || DASH}</Tag>
                {n.impact === "HIGH" && <Tag color={C.saffron} solid>HIGH IMPACT</Tag>}
                {n.actionable && <Tag color={C.gold}>ACTIONABLE</Tag>}
              </div>
              <span className="mono" style={{ fontSize: 9.5, color: C.dim, letterSpacing: ".06em" }}>{n.tag}</span>
            </div>
            <div style={{ fontSize: 12.5, color: C.text, marginTop: 6, lineHeight: 1.55 }}>
              {n.link ? (
                <a href={n.link} target="_blank" rel="noopener noreferrer" style={{ color: C.text, textDecoration: "none" }}>
                  {n.head} <span style={{ color: C.saffron, fontSize: 10 }}>↗</span>
                </a>
              ) : n.head}
            </div>
          </div>
            ...
            <div key={i} style={{ padding: "12px 14px", borderBottom: `1px solid ${C.lineSoft}` }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 8, flexWrap: "wrap" }}>
                <div style={{ fontSize: 17, fontWeight: 700 }}>
                  {c.sym} {c.strike || ""} <span style={{ color: c.side === "CE" ? C.bull : C.bear }}>{c.side}</span>
                </div>
              ...
              <div className="krt-cols2" style={{ marginTop: 8, gap: 10 }}>
                <div>
                  ...
                  <Row label="STATUS" value={c.status || DASH} color={C.saffron} />
                </div>
                <div>
              ...
              {c.advice && <div className="mono" style={{ fontSize: 10.5, color: C.saffron, marginTop: 8, lineHeight: 1.5 }}>{c.advice}</div>}
              {c.timeline && (
                <div className="mono" style={{ fontSize: 9.5, color: C.dim, marginTop: 8, display: "flex", gap: 12, flexWrap: "wrap" }}>
                  {c.timeline.filter((t) => t.t).map((t) => (
                    <span key={t.k}>{t.k} <span style={{ color: C.text }}>{t.t}</span></span>
                  ))}
                </div>
              )}
            </div>
      ...
      <Panel title="CALL LOG" pad={0} right={<span className="mono" style={{ fontSize: 9.5, color: C.dim }}>{today.length}</span>}>
        {today.length ? today.map((c, i) => (
          <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr 60px 70px 70px 90px", gap: 8, alignItems: "center", padding: "9px 14px", borderBottom: `1px solid ${C.lineSoft}` }}>
            <span style={{ fontSize: 12 }}>
              {c.sym} <span style={{ color: c.side === "CE" ? C.bull : C.bear }}>{c.side}</span>
            </span>
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      <Panel title="FEED HEALTH">
        ...
        <Row label="BROKER ERROR" value={(diag && diag.broker_error) || "none"} color={diag && diag.broker_error ? C.bear : C.dim} />
        ...
        <Row label="LAST SCAN" value={(snap && snap.updated) || DASH} />
        <Row label="SCAN TIME" value={has(snap && snap.scan_ms) ? `${intl(snap.scan_ms)} ms` : DASH} />
        <Row label="SCAN ERROR" value={h.scan_error || (diag && diag.scan_error) || "none"} color={h.scan_error ? C.bear : C.dim} />
        <Row label="TELEGRAM ALERTS" value={has(diag && diag.telegram) ? (diag.telegram ? "ON" : "OFF") : DASH} />
        <Row label="SERVER TIME (IST)" value={(diag && diag.server_time) || DASH} />
      </Panel>

      {diag && diag.storage_warning && (
        <Panel pad={12} style={{ background: `${C.saffron}12`, border: `1px solid ${C.saffron}55` }}>
          <div className="mono" style={{ fontSize: 10.5, color: C.saffron, lineHeight: 1.5 }}>⚠ {diag.storage_warning}</div>
        </Panel>
      )}
          ...
          <header style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap" }}>
              ...
              {feed.indices.length ? feed.indices.map((x) => (
                <div key={x.name} className="mono" style={{ fontSize: 10.5 }}>
                  <span style={{ color: C.dim }}>{x.name} </span>
                  <span>{num(x.ltp)} </span>
...
try {
  ...
  if (el) {
    el.innerHTML =
      '<div style="padding:20px;font-family:ui-monospace,monospace;color:#E8EAF0">' +
      '<div style="color:#FFA033;letter-spacing:.26em;font-size:10px">KRT AI</div>' +
      '<div style="font-size:16px;font-weight:700;margin:8px 0 12px">The terminal could not start.</div>' +
      '<div style="font-size:11px;color:#FF4D5E;background:rgba(255,77,94,.07);border:1px solid rgba(255,77,94,.33);' +
      'border-radius:3px;padding:12px;line-height:1.6;white-space:pre-wrap;word-break:break-word"></div></div>';
    el.querySelector('div > div:last-child').textContent = String((err && err.stack) || err);
  }
  throw err;
}
